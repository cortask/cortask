...
def main() -> int:
    ...
    ap.add_argument("--background", default="", help="Background (GPT models only): transparent, opaque, auto.")
    ap.add_argument("--output-format", default="", help="Output format (GPT models only): png, jpeg, webp.")
    ...
    if args.model == "dall-e-3" and count > 1:
        print(f"Warning: dall-e-3 only supports 1 image at a time. Reducing count from {count} to 1.", file=sys.stderr)
        count = 1
